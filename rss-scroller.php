	...
	//{
		...
		if ( ! is_wp_error( $rss ) )
		{
			...
			if ( $maxitems > 0 )
			{
				...
				<script src="<?php echo plugins_url(); ?>/rss-scroller/rss-scroller.js" type="text/javascript"></script>
	...
	//{
		...
		if ( ! is_wp_error( $rss ) )
		{
			...
			if ( $maxitems > 0 )
			{
				...
				$rss_scr = $rss_scr .'<script src="'.plugins_url().'/rss-scroller/rss-scroller.js" type="text/javascript"></script>';
	...
	<div class="wrap">
	  <div class="form-wrap">
		...
		if (isset($_POST['rss_scr_form_submit']) && $_POST['rss_scr_form_submit'] == 'yes')
		{
			...
			$rss_scr_title = stripslashes(sanitize_text_field($_POST['rss_scr_title']));
			$rss_scr_width = stripslashes(intval($_POST['rss_scr_width']));
			$rss_scr_height = stripslashes(intval($_POST['rss_scr_height']));
			$rss_scr_delay = stripslashes(intval($_POST['rss_scr_delay']));
			$rss_scr_speed = stripslashes(intval($_POST['rss_scr_speed']));
			$rss_scr_num = stripslashes(intval($_POST['rss_scr_num']));
			$rss_scr_url = stripslashes(esc_url_raw($_POST['rss_scr_url']));
			$rss_scr_url2 = stripslashes(esc_url_raw($_POST['rss_scr_url2']));
			$rss_scr_url3 = stripslashes(esc_url_raw($_POST['rss_scr_url3']));
			
			if(!is_numeric($rss_scr_width) || $rss_scr_width == 0) { $rss_scr_width = 300; }
			if(!is_numeric($rss_scr_height) || $rss_scr_height == 0) { $rss_scr_height = 200; }
			if(!is_numeric($rss_scr_delay) || $rss_scr_delay == 0) { $rss_scr_delay = 5000; }
			if(!is_numeric($rss_scr_speed) || $rss_scr_speed == 0) { $rss_scr_speed = 5; }
			if(!is_numeric($rss_scr_num) || $rss_scr_num == 0) { $rss_scr_num = 10; }
